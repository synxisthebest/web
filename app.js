...
function setupEventListeners() {
    ...
    const btnAdminPanel = document.getElementById('btn-admin-panel');
    if (btnAdminPanel) {
        btnAdminPanel.onclick = () => openAdminPanel();
    }

    const btnAdminAdd = document.getElementById('btn-admin-add');
    if (btnAdminAdd) {
        btnAdminAdd.onclick = () => handleAdminTransaction('add');
    }

    const btnAdminSubtract = document.getElementById('btn-admin-subtract');
    if (btnAdminSubtract) {
        btnAdminSubtract.onclick = () => handleAdminTransaction('subtract');
    }

    // Theme Toggle action
    const themeBtn = document.getElementById('btn-theme-toggle');
    if (themeBtn) {
        // Khởi tạo theme đã lưu
        const currentTheme = localStorage.getItem('royal_theme') || 'dark';
...
function refreshAdminUsers() {
    ...
    if (!userSelect || !usersList) return;
    ...
    users.forEach(user => {
        ...
        opt.textContent = `${user.username} (${formatMoney(user.balance)}đ)`;
        ...
        const roleBadge = user.isAdmin ? '<span style="color:#a855f7; font-weight:700; margin-left: 5px;">[Admin]</span>' : '';
        item.innerHTML = `
            <span>${user.username}${roleBadge}</span>
            <span style="color:var(--accent-gold); font-weight:600;">${formatMoney(user.balance)}đ</span>
        `;
        usersList.appendChild(item);
    });
}

function handleAdminTransaction(type) {
    const targetUsername = document.getElementById('admin-user-select').value;
    const amount = parseInt(document.getElementById('admin-add-amount').value) || 0;
    ...
    if (type === 'add') {
        ...
        showToast(`Cộng thành công +${formatMoney(amount)}đ cho tài khoản ${targetUsername}!`);
    } else if (type === 'subtract') {
        if (users[userIndex].balance < amount) {
            showToast(`Không đủ số dư để trừ! Số dư hiện tại: ${formatMoney(users[userIndex].balance)}đ`, "error");
            return;
        }
        users[userIndex].balance -= amount;
        showToast(`Trừ thành công -${formatMoney(amount)}đ khỏi tài khoản ${targetUsername}!`);
    }
    
    localStorage.setItem('royal_users', JSON.stringify(users));
    
    // Nếu đối tượng được điều chỉnh chính là tài khoản admin đang đăng nhập
